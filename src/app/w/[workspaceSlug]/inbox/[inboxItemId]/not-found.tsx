...
export default async function InboxItemDetailNotFound() {
  const actor = await getAuthenticatedActor();
  const language = actor ? await getPersistedDashboardLanguage(actor.userId) : "en";
  const labels = getInboxDetailLabels(getDashboardLabels(language));
  return (
    <main className="mx-auto flex min-h-[52vh] w-full max-w-190 flex-col justify-center px-4 py-10 sm:px-6">
      ...
      <Link className="mt-6 inline-flex w-fit items-center rounded-[8px] bg-[#2563eb] px-3.5 py-2.5 text-[13px] font-medium text-white transition-colors hover:bg-[#1d4ed8] focus-visible:outline-2 focus-visible:outline-offset-3 focus-visible:outline-[#2563eb]" href="../">{labels.back}</Link>
    </main>
  );
}
