...
export function mapCreateAccountFailure(code: string | undefined): CreateAccountFailure {
  switch (code) {
    case "INVALID_ACCOUNT_NAME": return { code, field: "name" };
    case "INVALID_ACCOUNT_TYPE": return { code, field: "type" };
    case "INVALID_CURRENCY": return { code, field: "currency" };
    case "INVALID_OPENING_BALANCE": return { code, field: "openingBalance" };
    case "WORKSPACE_FORBIDDEN": return { code };
    case "UNAUTHENTICATED":
    case "ACCOUNT_CREATE_FAILED": return { code };
    default: return { code: "ACCOUNT_CREATE_FAILED" };
  }
}
...
export function canAttachCreatedAccountToWorkspace(requestWorkspaceId: string, currentWorkspaceId: string): boolean {
