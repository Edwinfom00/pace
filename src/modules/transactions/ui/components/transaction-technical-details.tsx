  ...
  return (
    <details className="group rounded-[12px] border border-[#e6eaf0] bg-white">
      ...
      <dl className="grid gap-x-6 gap-y-3 border-t border-[#edf0f4] px-4 py-4 text-[12px] sm:grid-cols-2 sm:px-5">
        <div><dt className="text-[#71809a]">Transaction ID</dt><dd className="mt-1 break-all font-mono text-[#34405d]">{transaction.id}</dd></div>
        <div><dt className="text-[#71809a]">Created</dt><dd className="mt-1 text-[#34405d]"><time dateTime={transaction.createdAt}>{formatDetailTimestamp(transaction.createdAt, locale, timeZone)}</time></dd></div>
        <div><dt className="text-[#71809a]">Last updated</dt><dd className="mt-1 text-[#34405d]"><time dateTime={transaction.updatedAt}>{formatDetailTimestamp(transaction.updatedAt, locale, timeZone)}</time></dd></div>
      </dl>
    </details>
  );
}
