...
function AccountsBalanceSummary({
  ...
  return (
    <section
      aria-labelledby="accounts-balance-summary"
      className="rounded-[12px] border border-[#e4e9f0] bg-white p-5 sm:p-6"
    >
      <h2
        className="text-[17px] font-semibold tracking-[-0.02em] text-[#14203a]"
        ...
        <div className="mt-6 grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
          {overview.summary.map((total) => (
            ...
            >
              <span
                aria-hidden="true"
                className="grid size-12 shrink-0 place-items-center rounded-[13px] bg-[#eef4ff] text-[#2563eb]"
              >
                <FiDollarSign className="size-5" />
              </span>
              <dl className="min-w-0">
                <dt className="text-[12px] font-medium text-[#71809a]">
                  {total.currency}
                </dt>
                <dd className="mt-0.5 truncate text-[22px] font-semibold tracking-[-0.04em] text-[#101a35]">
                  {formatOverviewMoney(
                    total.currentBalanceMinor,
                    total.currency,
                    locale,
                  )}
                </dd>
                <dd className="mt-1 text-[12px] text-[#71809a]">
                  {formatActiveAccountCount(
                    labels.summaryAccounts,
                    total.accountCount,
                  )}
                </dd>
              </dl>
